...
func TestPutGetSandbox_Roundtrip(t *testing.T) {
	...
	if got.nimbusID != "wl-1" {
		t.Errorf("nimbusID = %q, want wl-1", got.nimbusID)
	}
...
func TestPersistence_AcrossReload(t *testing.T) {
	dir := t.TempDir()
	s1 := newFileStore(dir)
	s1.putSandbox(&sandboxRecord{id: "sb-persist", nimbusID: "wl-p", createdAt: time.Unix(10, 0), state: runtimeapi.PodSandboxState_SANDBOX_NOTREADY})
	...
	if got.nimbusID != "wl-p" {
		t.Errorf("nimbusID = %q after reload, want wl-p", got.nimbusID)
	}
