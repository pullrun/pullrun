...
// NewKernelCommand returns the parent `nimbusctl kernel` command.
//
// Subcommands:
...
// `~/.nimbus/kernels/vmlinux-<ver>` by default.
//
...
~/.nimbus/kernels/vmlinux-<version>.

Once installed, point nimbus at the kernel via the
NIMBUS_KERNEL_PATH environment variable when starting
nimbus-runtime, or pass --kernel-path to the smoke binary.

Examples:
  # Download Kata's arm64 kernel for Linux 3.31.0
  nimbusctl kernel install --version 3.31.0

  # Copy a kernel you already have on disk
  nimbusctl kernel install --from /path/to/vmlinux.container

  # Install Kata's amd64 kernel on an Intel Mac
  nimbusctl kernel install --version 3.31.0 --arch amd64`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if destPath == "" {
				...
				if fromLocal != "" {
					destPath = filepath.Join(home, ".nimbus", "kernels", filepath.Base(fromLocal))
				} else {
					destPath = filepath.Join(home, ".nimbus", "kernels", fmt.Sprintf("vmlinux-%s", version))
				}
			}
			...
			fmt.Fprintf(cmd.OutOrStdout(), "\nTo use it, set the env var when starting nimbus-runtime:\n  NIMBUS_KERNEL_PATH=%s nimbus-runtime ...\n", vmlinuxPath)
			return nil
		},
	}
	...
	cmd.Flags().StringVar(&dest, "dest", "", "Destination file path (default: ~/.nimbus/kernels/vmlinux-<ver>)")
