...
//! (`nimbus-oci` + `MmapStore`) and the Apple Virtualization
...
//! just another OCI artifact in nimbus's content-addressed
...
//! # Kernel image layout
//!
//! A nimbus kernel image is a regular OCI image with a
//! well-known shape. The layer tarball(s) contain:
//!
//! ```text
//! /boot/vmlinux
//! /boot/initramfs.cpio.gz    (optional)
//! /usr/lib/nimbus/nimbus-runtime  (optional, for the runtime; not consumed by v0)
//! ```
//!
...
/// Well-known path inside a nimbus kernel image where the
...
impl StagedKernel {
    ...
    /// Pull a kernel OCI image via the standard `nimbus-oci`
    ...
    pub async fn from_image_with_insecure(
        ...
        if !vmlinux.exists() {
            return Err(OciKernelError::MissingFile(format!(
                "{} (image is missing /boot/vmlinux — is this a nimbus kernel image?)",
                vmlinux.display()
            )));
        }
