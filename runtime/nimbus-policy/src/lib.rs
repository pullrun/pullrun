...
pub use nimbus_store::{Digest as BlobDigest, MmapStore};
...
impl From<nimbus_store::StoreError> for PolicyError {
    fn from(e: nimbus_store::StoreError) -> Self {
...
impl PolicyEngine {
    ...
    pub fn evaluate_for_image(
        ...
        if let Some(profile) = &policy.seccomp_profile {
            if profile != "default" && profile != "unconfined" && !profile.starts_with("nimbus:") {
                return Ok(PolicyDecision::Deny(format!(
    ...
    fn check_signature(
        ...
        let sig_mmap = match store.get_blob(&sig_digest) {
            Ok(m) => m,
            Err(nimbus_store::StoreError::NotFound(_)) => return Ok(SignatureCheck::Missing),
...
mod tests {
    ...
    fn test_signature_required_with_keys_allows_in_evaluate() {
        ...
        let dir = std::env::temp_dir().join("nimbus-test-policy-sig-roundtrip");
    ...
    fn test_seccomp_profile_default_ok() {
        ...
        let store = Arc::new(MmapStore::new(std::env::temp_dir().join("nimbus-test-policy")));
        let d = engine
