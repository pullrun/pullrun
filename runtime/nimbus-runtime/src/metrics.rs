//! Prometheus metrics for the Nimbus runtime.
//!
...
//! |---|---|---|---|
//! | `nimbus_pulls_total` | counter | `registry`, `status` | Image pulls, by outcome |
//! | `nimbus_pull_duration_seconds` | histogram | — | Wall time of `PullImage` |
//! | `nimbus_workloads_started_total` | counter | `backend` | Workloads that successfully started |
//! | `nimbus_workloads_exited_total` | counter | `backend`, `exit_code` | Workloads that exited (via stop) |
//! | `nimbus_workloads_running` | gauge | `backend` | Current count of running workloads |
//! | `nimbus_workload_start_duration_seconds` | histogram | — | Wall time of `RunWorkload` (request → started) |
//! | `nimbus_store_nodes` | gauge | — | DAG nodes currently in the in-process cache |
//! | `nimbus_store_bytes` | gauge | — | Total mmap'd bytes across cached nodes |
//! | `nimbus_build_info` | gauge | `version` | Build metadata, always `1` |
//!
//! ## HTTP endpoint
//!
...
pub mod names {
    pub const PULLS_TOTAL: &str = "nimbus_pulls_total";
    pub const PULL_DURATION_SECONDS: &str = "nimbus_pull_duration_seconds";
    pub const WORKLOADS_STARTED_TOTAL: &str = "nimbus_workloads_started_total";
    pub const WORKLOADS_EXITED_TOTAL: &str = "nimbus_workloads_exited_total";
    pub const WORKLOADS_RUNNING: &str = "nimbus_workloads_running";
    pub const WORKLOAD_START_DURATION_SECONDS: &str = "nimbus_workload_start_duration_seconds";
...
/// RAII timer for `nimbus_workload_start_duration_seconds`. Same shape
...
mod tests {
    ...
    #[tokio::test]
    async fn render_after_increment_contains_nimbus_prefix() {
        ...
        let body = h.render();
        assert!(body.contains("nimbus_pulls_total"), "missing pulls_total in render");
        assert!(
            body.contains("nimbus_workloads_started_total"),
            "missing workloads_started_total in render"
        );
        assert!(
            body.contains("nimbus_workloads_exited_total"),
            "missing workloads_exited_total in render"
        );
        assert!(body.contains("nimbus_workloads_running"));
        assert!(body.contains("nimbus_store_nodes"));
        assert!(body.contains("nimbus_store_bytes"));
        assert!(body.contains("nimbus_build_info"));
    }
