...
//! - `firecracker` binary is not on PATH (or `NIMBUS_FC_BIN` env var unset)
//! - `NIMBUS_FC_VMLINUX` env var is unset (no vmlinux staged)
//!
//! ## Required staging
//!
//! The CI / dev environment must stage:
//! - `NIMBUS_FC_BIN`     — path to a firecracker v1.10+ binary
//! - `NIMBUS_FC_VMLINUX` — path to an uncompressed vmlinux ELF that
...
//! export NIMBUS_FC_BIN=/usr/local/bin/firecracker
//! export NIMBUS_FC_VMLINUX=$vmlinux
//! ```
//!
//! Run with: `cargo test -p nimbus-vm --test firecracker_boot -- --include-ignored --nocapture`
...
fn skip_if_unavailable() -> Option<String> {
    ...
    if !std::path::Path::new(&bin).exists() && which(&bin).is_none() {
        return Some(format!(
            "firecracker binary not found (NIMBUS_FC_BIN={bin})"
        ));
    }
...
/// script. Override the source with `NIMBUS_FC_ROOTFS_TAR` to test
/// offline.
...
#[tokio::test]
#[ignore = "requires /dev/kvm, firecracker binary, NIMBUS_FC_VMLINUX, and network for alpine rootfs"]
async fn firecracker_boot_smoke() {
    ...
    let stage_dir =
        std::env::var("NIMBUS_FC_STAGE").unwrap_or_else(|_| "/tmp/fc-smoke-stage".into());
