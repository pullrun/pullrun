...
mod tests {
    ...
    use nimbus_policy::cosign::{self, SignatureBlob};
    use nimbus_policy::{CosignKey, Policy, PolicyEngine};
    use nimbus_runtime::service::{RuntimeCommand, ServiceConfig};
    ...
    fn make_service(dir: &PathBuf, policy: Policy, keys: Vec<CosignKey>) -> nimbus_runtime::service::RuntimeService {
    ...
    fn fresh_dir(name: &str) -> PathBuf {
        let dir = std::env::temp_dir().join(format!("nimbus-runtime-policy-{name}"));
    ...
    async fn test_signature_required_unsigned_denies() {
        ...
        assert!(matches!(decision, nimbus_policy::PolicyDecision::Deny(_)));
    }
    ...
    async fn test_signature_required_with_signed_blob_allows() {
        ...
        let decision = engine
            ...
            .unwrap();
        assert_eq!(decision, nimbus_policy::PolicyDecision::Allow);
    }
    ...
    async fn test_max_cvss_violation_denies() {
        ...
        let manifest = "abc123";
        let blob = nimbus_policy::sbom::SbomBlob {
            format: "cyclonedx-1.5".into(),
            components: vec![],
            vulnerabilities: vec![nimbus_policy::sbom::Vulnerability {
                id: "CVE-2024-9999".into(),
                component: "openssl".into(),
                cvss: 9.8,
            }],
        };
        let bytes = nimbus_policy::sbom::encode_sbom(&blob).unwrap();
        let digest = nimbus_policy::sbom::sbom_digest_for(manifest);
        store.put_blob_blocking(&digest, &bytes).unwrap();
        ...
        let decision = engine
            ...
            .unwrap();
        match decision {
            nimbus_policy::PolicyDecision::Deny(reason) => {
