...
mod tests {
    ...
    async fn test_dns_record_management() {
        ...
        proxy.add_record("test.pullrun.local", "10.42.0.5").await;
        let result = proxy.resolve_local("test.pullrun.local").await;
        assert_eq!(result, Some("10.42.0.5".to_string()));
        proxy.remove_record("test.pullrun.local").await;
        let result = proxy.resolve_local("test.pullrun.local").await;
