...
//! the host (`pullrun-runtime`) and the guest (`pullrun-init`) speak this
...
//!
//! ```text
//!  guest (pullrun-init)         host (pullrun-runtime)
//!       |                              |
//!       |  InitHello                   |
//!       |----------------------------->|
//!       |                              |
...
pub enum FrameType {
    ...
    /// Host → guest. Update the PTY window size (rows, cols).
    /// Ignored if the workload doesn't have a PTY.
    WindowSize = 0x09,
}
...
/// A VirtioFS mount entry shared from the host into the guest.
///
/// Each mount corresponds to a host directory shared via a
/// `VZVirtioFileSystemDeviceConfiguration` with a unique tag.
/// The guest mounts it at `destination` before spawning the
/// workload.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct VsockMount {
    /// VirtioFS share tag — must match the tag on the host's
    /// `VZVirtioFileSystemDeviceConfiguration` (e.g. `"pullrun-vol-0"`).
    pub tag: String,
    /// Absolute path inside the guest where the share is mounted.
...
pub enum Frame {
    ...
    /// `pullrun-init` itself (for diagnostics).
    InitHello {
        /// Workload identifier (UUID or workload id).
        workload_id: String,
        /// PID of pullrun-init, for diagnostics.
        init_pid: u32,
    },
    ...
    WorkloadSpec {
        ...
        /// `pullrun-init`'s default (typically `/`).
        working_dir: String,
        /// If true, allocate a PTY (pseudo-terminal) for the
        /// workload so it sees a real terminal (interactive
        /// shell with prompt, tab-completion, job control).
        tty: bool,
        /// Initial terminal rows (0 means use default 24).
        rows: u32,
        /// Initial terminal columns (0 means use default 80).
        cols: u32,
        /// VirtioFS volume mounts to set up before the workload runs.
        mounts: Vec<VsockMount>,
    },
    ...
    /// Host → guest. Update the PTY window size.
    WindowSize {
...
impl Frame {
    ...
    pub fn frame_type(&self) -> FrameType {
        match self {
            ...
            Self::WindowSize { .. } => FrameType::WindowSize,
        }
    }
    ...
    fn encode_payload(&self) -> Bytes {
        match self {
            ...
            Self::WorkloadSpec { command, env, working_dir, tty, rows, cols, mounts } => {
                ...
                buf.put_u8(if *tty { 1 } else { 0 });
                buf.put_u32(*rows);
                buf.put_u32(*cols);
                buf.put_u32(mounts.len() as u32);
                for m in mounts {
                    write_length_prefixed_string(&mut buf, &m.tag);
                    write_length_prefixed_string(&mut buf, &m.destination);
                    buf.put_u8(if m.read_only { 1 } else { 0 });
                }
                buf.freeze()
            }
    ...
    fn decode_payload(ty: FrameType, payload: Bytes) -> Result<Self, ProtocolError> {
        ...
        Ok(match ty {
            ...
            FrameType::WindowSize => {
                ...
                let rows = cur.get_u16();
                let cols = cur.get_u16();
                Self::WindowSize { rows, cols }
            }
        })
    }
...
/// `pullrun-vm::attach::DEFAULT_VSOCK_PORT` for the
