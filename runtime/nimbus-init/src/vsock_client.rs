...
//! (default: 42).
//!
//! In dev/test mode, we may also run `nimbus-init` outside a
...
impl VsockClient {
    ...
    pub async fn connect(_port: u32) -> Result<Self, VsockError> {
        ...
        // Unix fallback.
        let sock_path = std::env::var("NIMBUS_VSOCK_FALLBACK_UNIX")
            .unwrap_or_else(|_| "/tmp/nimbus-init.sock".to_string());
    ...
    pub async fn send_frame(&self, frame: Frame) -> Result<(), VsockError> {
        let bytes = nimbus_vsock::encode(&frame);
    ...
    pub async fn recv_frame(&self) -> Result<Frame, VsockError> {
        ...
        let ty_byte = hdr[4];
        if len > nimbus_vsock::MAX_PAYLOAD as usize {
        ...
        nimbus_vsock::decode(&payload, ty).map_err(VsockError::Encode)
    }
...
mod tests {
    ...
    /// nimbus_vsock's `read_frame`.
    async fn read_frame_from(stream: &mut UnixStream) -> Frame {
        nimbus_vsock::read_frame(stream).await.unwrap()
    }
    ...
    async fn unix_fallback_roundtrip() {
        ...
        let sock_path = dir.path().join("nimbus-init.sock");
        std::env::set_var("NIMBUS_VSOCK_FALLBACK_UNIX", &sock_path);
    ...
    async fn unix_fallback_bidi() {
        ...
        let host_task = tokio::spawn(async move {
            ...
            // Send WorkloadSpec
            let spec = nimbus_vsock::Frame::WorkloadSpec {
                command: vec!["/bin/echo".into(), "hello".into()],
                env: vec!["FOO=bar".into()],
                working_dir: "/".to_string(),
            };
            let bytes = nimbus_vsock::encode(&spec);
        ...
        match spec {
            Frame::WorkloadSpec { command, env, working_dir } => {
