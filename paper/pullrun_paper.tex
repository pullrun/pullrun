...
\begin{itemize}[leftmargin=*]
    ...
    \item \textbf{Section~\ref{sec:mcp}:} The native MCP server, its tool catalogue (15+ tools in v0.3.2), transport modes, and integration with AI coding agents.
...
\subsection{Known Limitations in v0.3.2}
...
The server exposes 15 tools across three categories in v0.3.2, summarized in \Cref{tab:mcp-tools}. The \texttt{compose\_up} and \texttt{compose\_down} tools, previously declared but CLI-only, are now fully implemented via the MCP API.

\begin{table}[htbp]
    \centering
    \caption{MCP tool catalogue exposed by \pullrun{} 0.3.2.}
    \label{tab:mcp-tools}
...
The \texttt{compose\_up} and \texttt{compose\_down} operations are now fully supported via the MCP API as of v0.3.2, completing the full lifecycle coverage for AI agents managing multi-service stacks.
...
\begin{table}[htbp]
    \centering
    \caption{Workload boot latency by backend (v0.3.2; Firecracker boot improved in v0.3.2 via cached ext4 image reuse).}
    \label{tab:boot}
    \begin{tabular}{@{}lrrr@{}}
        \toprule
...
Apple Virtualization's low boot time reflects the tight integration between the guest kernel and the hypervisor on Apple Silicon. Firecracker boot time improved significantly in v0.3.2 ($\sim$500 ms, down from $\sim$1.24 s in v0.1.0) through ext4 image caching: the ext4 filesystem is built once per image digest and reused on subsequent boots, eliminating the \texttt{mkfs.ext4} step from the critical path. The WSL2 path adds approximately 30 ms of socket overhead. Note that Apple Virt boot time includes rootfs materialization (hardlink creation, $\sim$40 ms); the VM creation via \texttt{VZVirtualMachine::start()} completes in $\sim$120 ms.
...
\begin{table}[htbp]
    \centering
    \caption{Feature comparison: \pullrun{} 0.3.2 vs Docker CE 26 vs Podman 5.x.}
...
\pullrun{} 0.3.2 is open-source and available at \url{https://github.com/pullrun/pullrun}. The source code archived at the version corresponding to this technical report is permanently available via Zenodo~\cite{zenodo} at \url{https://doi.org/10.5281/zenodo.20679669}.
...
\bibitem{zenodo}
M.~Boukaba, ``Pullrun v0.3.2: A Content-Addressed Workload Execution System,''
Zenodo, June 2026.
\newblock DOI: \href{https://doi.org/10.5281/zenodo.20679669}{10.5281/zenodo.20679669}
