...
//! `NIMBUS_FC_BIN`, `NIMBUS_FC_VMLINUX`, and `NIMBUS_FC_ROOTFS_TAR`
//! env vars are read.
//!
//! Run with: `cargo test -p nimbus-vm --test firecracker_network -- --include-ignored --nocapture`
...
use nimbus_vm::{create_tap, ensure_bridge, teardown_tap, GATEWAY_IP, NETMASK};
...
#[tokio::test]
#[ignore = "requires /dev/kvm, firecracker binary, NIMBUS_FC_VMLINUX, /dev/net/tun (CAP_NET_ADMIN), and network for alpine rootfs"]
async fn firecracker_network_smoke() {
    ...
    let stage_dir =
        std::env::var("NIMBUS_FC_STAGE_NET").unwrap_or_else(|_| "/tmp/fc-net-stage".into());
