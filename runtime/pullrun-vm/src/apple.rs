...
//!   a static `pullrun-runtime` binary inside the initramfs,
...
/// via the `PULLRUN_APPLE_VIRT_CALLBACK_TIMEOUT_SECS` env var
...
fn callback_timeout_secs() -> u64 {
    ...
    *OVERRIDE.get_or_init(|| {
        std::env::var("PULLRUN_APPLE_VIRT_CALLBACK_TIMEOUT_SECS")
            .ok()
...
pub struct AppleVirtPoolConfig {
    ...
    /// pulls a pullrun kernel image via the standard OCI
    /// pipeline) or by
    ...
    /// (`/var/lib/pullrun`). In tests it can be any directory.
...
fn build_vm(
    ...
    // `pullrun-store` is what the guest will reference.
    let store_tag = NSString::from_str("pullrun-store");
