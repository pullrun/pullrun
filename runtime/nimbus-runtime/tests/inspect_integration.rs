...
mod tests {
    ...
    use nimbus_runtime::proto::runtime_server::Runtime;
    use nimbus_runtime::service::{RuntimeCommand, ServiceConfig};

    fn fresh_dir(name: &str) -> PathBuf {
        let dir = std::env::temp_dir().join(format!("nimbus-runtime-inspect-{name}"));
    ...
    async fn inspect_unknown_workload_returns_found_false() {
        ...
        let req = tonic::Request::new(nimbus_runtime::proto::InspectRequest {
    ...
    async fn inspect_known_workload_returns_full_snapshot() {
        ...
        let network_rule = nimbus_net::NetworkRule {
...

        {
            ...
            map.insert(
                id.clone(),
                nimbus_runtime::service::WorkloadState {
    ...
    async fn event_to_proto_conversion_preserves_fields() {
        ...
        let ev = nimbus_runtime::Event::new("wl-1", nimbus_runtime::EventKind::WorkloadStarted)
            .with_metadata("backend", "container")
            .with_metadata("pid", "1234");
        let proto: nimbus_runtime::proto::Event = ev.clone().into();
