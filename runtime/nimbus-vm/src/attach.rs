...
//!    and waits for the `nimbus-init` guest to connect on
...
//! stream. `nimbus-vm` itself stays unaware of gRPC or proto
//! types; the contract is just "two byte streams that produce
//! and consume length-prefixed [`nimbus_vsock::Frame`]s."
//!
...
/// The default port the `nimbus-init` guest connects to on
...
/// connection to its `nimbus-init` guest.
///
...
pub struct AppleVirtAttachHandle {
    ...
    pub init_hello: nimbus_vsock::Frame,
}
