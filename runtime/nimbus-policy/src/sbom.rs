...
mod tests {
    ...
    fn test_evaluate_sbom_missing() {
        let dir = std::env::temp_dir().join("nimbus-policy-sbom-missing");
    ...
    fn test_evaluate_sbom_present() {
        let dir = std::env::temp_dir().join("nimbus-policy-sbom-present");
        let _ = std::fs::remove_dir_all(&dir);
        let store = Arc::new(MmapStore::new(dir));
